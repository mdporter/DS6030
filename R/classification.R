...
## Michael D. Porter
## Created: Sept 2021
## For: Data Mining (DS6030) at University of Virginia
...
# but categorical/factor data needs to be handled separately and consistantly
...
rec = recipe(fmla, data=Default) %>% prep() 
X.train = rec %>% bake(new_data = Default, all_predictors())
Y.train = rec %>% bake(new_data = Default, all_outcomes()) %>% pull() 
# X.test = rec %>% bake(new_data = test, all_predictors())
